...
if (isset($_POST['delete_id'])) {
    ...
    $delete_sql = "DELETE FROM news_event WHERE id = '$delete_id'";
...
$sql = "SELECT id, classid, Title, Description, Date, Type, Image FROM news_event ";
// fire query
...
if(mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<div class="p-6">';
        echo '<p class=" text-sm font-mono text-gray-600 mt-2 ">Date: ' . $row["classid"].'</p>';
...
if(isset($_POST['Insert'])){
   ...
   $insert = mysqli_query($conn, "INSERT INTO `news_event` (Title, Description, Date, Type, Image,classid) VALUES ('$Title', '$Description', '$Date', '$Type', '$image_name','$classidtype')") or die('query failed');

   // Move uploaded image to desired location
   move_uploaded_file($image_tmp, "newsimage_img/$image_name");
...
<section class="bg-white my-24">
    <div class="container flex items-center justify-center  px-6 mx-auto">
        <form class="w-full max-w-md" action="" method="post" enctype="multipart/form-data">
            ...
            <h1 class="text-3xl font-serif">Upload News & Event</h1>
            </div>

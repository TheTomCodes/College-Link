...
<h1 class="font-serif text-2xl md:text-6xl text-center text-blue-700 mt-6 mb-6">Result</h1>

<div class="flex flex-wrap justify-center mx-auto ">
    <?php
   $sql = "SELECT id, filename, classid, upload_date, studentid, classname, Papertype FROM studentresult ";
   // fire query
   ...
   if ($_SERVER["REQUEST_METHOD"] == "POST") {
       if (isset($_POST['delete_id'])) {
           ...
           $delete_sql = "DELETE FROM studentresult WHERE id = '$delete_id'";
    ...
    ?>
            <div class=" w-96 mx-4 my-4 p-6 bg-white border border-gray-200 rounded-lg shadow text-start">
                ...
                <p class=" text-lg  font-mono text-gray-700">Date: <?php echo $row['studentid']; ?></p>
                <p class=" text-lg  font-mono text-gray-700">Date: <?php echo $row['classname']; ?></p>
                <p class=" text-lg  font-mono text-gray-700">Date: <?php echo $row['Papertype']; ?></p>
                <p class=" text-lg mb-8 font-mono text-gray-700">Date: <?php echo $row['upload_date']; ?></p>
                <a href="<?php echo $file_path; ?>" class=" w-full block bg-indigo-600 text-white font-serif py-2 px-4 rounded-md text-center transition duration-300 ease-in-out transform hover:bg-indigo-700 hover:scale-105">
                    Download
    ...
    <div class="container flex items-center justify-center  px-6 mx-auto">
        <form class="w-full max-w-md"method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
            ...
            <div class="flex justify-center mx-auto">
                <h1 class="text-3xl font-serif">UploadResult</h1>
            </div>
            <div class="relative flex items-center mt-4">
                ...
                <input type="text" name="studentid"  required class="block w-full py-3 font-serif text-xl text-black bg-white border-2 border-blue-700 rounded-lg px-11" placeholder="student id number">
            </div>
            <div class="relative flex items-center mt-4">
                <span class="absolute">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mx-3 text-blue-700">
            ...
            <div class="relative flex items-center mt-4">
                ...
                <select type="PaperType" name="Papertype" required  class="block w-full py-3 font-serif text-xl text-black bg-white border-2 border-blue-700 rounded-lg px-11" placeholder="PaperType">
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        if (!in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
                ...
                $Papertype = mysqli_real_escape_string($conn,($_POST['Papertype']));
                $classidtype = mysqli_real_escape_string($conn,($_POST['classidtype']));

              

                // Insert the file information into the database
                $sql = "INSERT INTO studentresult (filename, filesize, filetype, classid, studentid, classname, Papertype) VALUES ('$filename', $filesize, '$filetype','$classidtype','$studentid','$classname','$Papertype')";
